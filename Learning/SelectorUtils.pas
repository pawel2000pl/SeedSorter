...
uses
    SysUtils, Classes, math, FPImage, UniversalImage, CalcUtils, nearestvectors, Incrementations;
...
const
    ...
    ProgressLabel64 = '0%============25%=============50%=============75%===========100%';
    
type
    TColorTable = array[Byte, Byte, Byte] of Double; //128MB
...
procedure CreateColorTable(const SampleArray : TSampleArray; var ColorTable : TColorTable);
...
begin
    ...
    for x := 0 to Image.Width-1 do
        ...
        begin
            ...
            for i := 0 to 3 do
            begin    
                d := (x/(Image.Width-1)*CornerOrder[i, 0] + (1-x/(Image.Width-1))*(1-CornerOrder[i, 0])) * 
                     (y/(Image.Height-1)*CornerOrder[i, 1] + (1-y/(Image.Height-1))*(1-CornerOrder[i, 1]));
...
function GetCount(const SampleArray : TSampleArray) : QWord;
var
    r, g, b : Integer;
begin
    Result := 0;
    for r := 0 to 255 do
        for g := 0 to 255 do
            for b := 0 to 255 do
                Inc(Result, SampleArray[r, g, b].Count);
end;

procedure PutSample(const Value : Double; const r, g, b : Integer;  var ColorTable : TColorTable);
const
    ray = 16;
var
    i : Integer;
...
procedure Normalize(var ColorTable : TColorTable);
var
    r, g, b : Integer;  
    lv, hv : Double;
begin
...
begin
    ...
    for x := 0 to Image.Width-1 do
        ...
        begin
            c.AssignFPColor(Image.Colors[x, y]);
            if InRange(ColorTable[c.r, c.g, c.b], BorderMin, BorderMax) then
            begin
                Inc(Count);
                if ChangeColor then
    ...
    Result := Count/(Image.Width*Image.Height);
end;
